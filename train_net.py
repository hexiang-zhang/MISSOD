...
CLASS_NAMES =["1"]
# 数据集路径
DATASET_ROOT = '/mnt/sdd/zhanghexiang/coco/'
...
TRAIN_LABEL_JSON =  os.path.join(ANN_ROOT, 'instances_train2014_label_0.1.json')
TRAIN_UNLABEL_JSON = os.path.join(ANN_ROOT, 'instances_train2014_unlabel_0.1.json')
...
class BestCheckpointer(HookBase):
  ...
  def after_step(self):
    ...
    if curr_val > max_val:
        print("\n%s > %s要存\n"%(curr_val,max_val))
        self.trainer.storage.put_scalar('max_bbox/AP50', curr_val)
        self.trainer.checkpointer.save("model_best_{}".format(float(max_val)))
